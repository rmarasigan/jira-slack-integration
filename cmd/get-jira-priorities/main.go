...
func handler(ctx context.Context, request events.APIGatewayProxyRequest) (*events.APIGatewayProxyResponse, error) {
	...
	// 2. Fetch all the JIRA Issue Priorities.
	priorities, err := jiracfg.GetPriorities(ctx)
	if err != nil {
		utility.Error(err, "JIRA", "failed to fetch all JIRA Issue Priorities")

		// 2.1 Check if the error is "not found" and return an HTTP
		// BadRequest Status response.
		if strings.HasSuffix(err.Error(), "not found") {
			response, err := api.EncodeJSONString(api.SetErrorMessage(errors.New("issue priorities not found")))
			if err != nil {
	...
	response, err := api.EncodeJSONString(priorities)
	if err != nil {
		utility.Error(err, "JSONError", "failed to unmarshal the JSON-encoded data")
		return api.StatusInternalServer(), nil
	}
